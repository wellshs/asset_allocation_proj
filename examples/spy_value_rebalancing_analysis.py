...
def analyze_spy_value_rebalancing():
    ...
    try:
        spy = download_price_data("SPY", "2019-01-01", "2024-12-31")
    ...
    # Configure parameters (Bernstein recommendations)
    params = ValueRebalancingParameters(
        value_growth_rate=Decimal("0.07"),  # 7% annual (Bernstein conservative)
    ...
    print(f"  매수: {buy_actions}회")
    print(f"  매도: {sell_actions}회")
    ...
    print("  - 목표 성장률 7%는 Bernstein의 보수적 추정")
    print("  - 리스크 관리를 중시하는 투자자에게 적합")

    return spy, result


def main():
    result = analyze_spy_value_rebalancing()
